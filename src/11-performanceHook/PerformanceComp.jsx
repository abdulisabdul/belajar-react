...
export default function PerformanceComp() {
  ...
  function addToCart() { setCart((prev) => prev + 1) }
  ...
  const _getData = () => {
    // ceritanya ini fetch data dari server 
    return [
      {
        nama_barang: "Macbook",
        harga: 1000000,
  ...
  const listProduct = useMemo(() => _getData(), [])
  const _addToCart = useCallback(addToCart,[])

  const user = {nama : 'abdul'}
